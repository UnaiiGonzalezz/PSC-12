...
public class CarritoService {
    ...
    public double getTotal(Cliente cliente) { return getOrCreateCarrito(cliente).getTotal(); }
    ...
    public CheckoutResponseDTO checkout(Cliente cliente) {
        ...
        if (med.getStock() == 0) med.setDisponible(false);
        ...
        var itemsDTO = carrito.getItems().stream()
                .map(ci -> new CheckoutResponseDTO.ItemDTO(
                        ci.getMedicamento().getNombre(),
                        ci.getMedicamento().getPrecio(),
                        ci.getCantidad(),
                        ci.getSubtotal()))
        ...
        return new CheckoutResponseDTO(
                compra.getId(), compra.getFechaCompra(),
                compra.getEstado(), compra.getPago(), itemsDTO);
    }


    /* ---------- Métodos internos (sobrecarga para reutilizar) ---------- */
